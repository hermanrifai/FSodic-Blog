...
$user = mysql_num_rows(mysql_query("SELECT * FROM user WHERE email = '".mysql_real_escape_string($log)."' OR username = '".mysql_real_escape_string($log)."'"));
if($user = 0)
{
...
{
$match = mysql_fetch_array(mysql_query("SELECT * FROM user WHERE username = '".mysql_real_escape_string($log)."' OR email = '".mysql_real_escape_string($log)."'"));
